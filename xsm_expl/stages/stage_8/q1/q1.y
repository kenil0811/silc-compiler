...
%token PLUS MINUS MUL DIV MOD LT GT LTE GTE NE EQ AND OR TYPE ENDTYPE NULLTOKEN CLASS ENDCLASS SELF NEW EXTENDS
...
//classes
ClassDefBlock	: CLASS ClassDefList ENDCLASS	{fprintf(out, "CALL MAIN\n"); $$ = $2; currclassptr = NULL;}
				|	{}
				;

ClassDefList	: ClassDefList ClassDef	{setVirtualFunctionTable(currclassptr, out); $2->mid=$1; $$=$2;}
				| ClassDef	{setVirtualFunctionTable(currclassptr, out); $$=$1; }
				;

ClassDef	: Cname '{' DECL CFieldList CMethodDecl ENDDECL CMethodDef '}'	{$$=$7; func($7);}
			;

Cname	: VAR	{$$=$1; currclassptr=insertClass($1->varname, NULL);}
		| VAR EXTENDS VAR	{currclassptr = insertClass($1->varname, $3->varname);}
		;
...
ArgList	:	ArgList ',' expr	{$3->mid=$1; $$=$3; printf("---%s\n", $3->type->name);}
		|	expr	{ $$=$1; $1->mid=NULL; printf("----%s\n", $1->type->name);}
		|	{$$=NULL; printf("lo\n");}
		;		
...
FieldFunction	: SELF '.' VAR '(' ArgList ')'	{if(currclassptr == NULL) {
													printf("Use of self outside class not allowed\n"); exit(0); }
												$$ = makeClassFuncCallNode(currclassptr, "self", NULL, $3->varname, $5, currfunc);
												}
				| VAR '.' VAR '(' ArgList ')'	{struct Gsymbol *gentry = gLookup($1->varname);
												...
												$$ = makeClassFuncCallNode(gentry->ctype, $1->varname, NULL, $3->varname, $5, currfunc);
												}
				| SELF '.' VAR '.' VAR '(' ArgList ')' {if(currclassptr == NULL) {
														...
														$$ = makeClassFuncCallNode(currclassptr, "self", $3->varname, $5->varname, $7, currfunc);
														}
				;
